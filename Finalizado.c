...
char encriptacion(char* alm_str_org, char* tipo_key){ //Función que recibe un array de código morse y lo encripta 
  int h, i = 0, j, k = 0, m, z = 1; //Variables
  printf("El texto encriptado es:\n");
  while(alm_str_org[i] != '\0'){  //Ejecuta el bucle hasta que encuentre una posición vacía en el array
    for(h = 0; h < 27; h++){  //Recorrido para hallar la posición de la letra de encriptación a usar
      m = i;  //Se toma el valor de i cada vez para reiniciar el recorrido de los tres caracteres
      for(j = 0; j < 3; j++){ //Se hace la comparación de los tres caracteres 
        if(alm_str_org[m] == '\0' && j ==1){  //Si hace falta completar los tres caracteres, se añade el segundo caracter como "X"
          alm_str_org[m] = 'X';
        }
        if(alm_str_org[m] == '\0' && j ==2){  //Si hace falta completar los tres caracteres, se añade el tercer caracter como "X"
          alm_str_org[m] = 'X';
        }
        if(alm_str_org[m] != orden_codif[h][j]){  //En caso un caracter no concuerde, se rompe este ciclo de comparación para ir hacia la siguiente letra
          z = 0;  //Variable que está actuando como boolean, hace que la letra evaluada no sea almacenada porque no es la correcta
          break;  //Se rompe el ciclo para evaluar la siguiente letra 
        }
        m++;
      }
      if(z){  //Pregunta si la variable boolean no ha sido modificada  
        string_encriptado[k] = tipo_key[h]; //Se añade la letra evaluada al array de respuesta
        break;  //Se rompe el ciclo porque ya se encontró la letra requerida
      }
      z = 1;  //Reinicia el boolean
    }
    i += 3; //i avanza tres posiciones porque ya se evalúo correctamente tres caracteres
    k++;
  }
  return 0;
}
...
char desmorsificar(char* alm_str_org){  //Función que recibe un array de código morse y lo convierte a texto
  ...
  while(alm_str_org[i] != '\0'){  //Ejecuta el bucle hasta que encuentre una posición vacía en el array
    ...
    y = i;  //Se almacena el valor de i en una auxiliar para su posterior uso
